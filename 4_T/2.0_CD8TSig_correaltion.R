...
dir.used <- "4_T/CD8T_cor/"
file.prefix <- "CD8T_cor_"

dataset <- as.data.frame(readxl::read_excel("../Datasets.xlsx"))
rownames(dataset) <- dataset$DataSets
CD8.path <- grep("CD8", unique(tcellSig$Pathway), value = T)
...
pathway_cor_list <- lapply(1:nrow(dataset), function(i) {
    ...
    seu_obj$sub_celltype <- ifelse((seu_obj$cd3 & seu_obj$cd8 & !(seu_obj$cd4)), "CD8_T",
                                 ifelse((seu_obj$cd3 & !(seu_obj$cd8) & seu_obj$cd4), "CD4T", "Unknown_T"))
    seu_obj <- subset(seu_obj, sub_celltype == "CD8_T")
    ...
    gene.list <- gene.list[names(gene.list) %in% c(names(metab_pas), CD8.path)]
    
    pathway_cor <- pathway_correlation(df = cbind(metab_pas, sig_pas), glist = gene.list)
    fastSave::saveRDS.pigz(pathway_cor, file = paste0("res/", dir.used, tumor, "_CD8_pathway_correlation.rds"))
    return(pathway_cor)
})
...
pathway_cor_list <- lapply(dataset$DataSets, function(tumor) {
    return(readRDS(paste0("res/4_T/", tumor, "_CD8_pathway_correlation.rds")))
})
...
get_pooled_data <- function(var) {
    res <- lapply(names(pathway_cor_list), function(tumor) {
        ...
        x <- data.frame(x[setdiff(rownames(x), CD8.path), CD8.path], check.names = FALSE)
...
column_annot <- rep(dataset$DataSets, each = 17)
pdf(paste0("plot/", dir.used, file.prefix, "heatmap_by_Datasets_changed_color.pdf"), width = 15, height = 5)
...
pdf(paste0("plot/", dir.used, file.prefix, "heatmap_by_Sig_changed_color.pdf"), width = 15, height = 7)
Heatmap(radj, col = circlize::colorRamp2(c(-0.5, 0, 0.5), myCols), 
        rect_gp = gpar(col = "white"), 
        cluster_columns = FALSE, cluster_rows = TRUE, 
        row_names_side = "left", show_row_dend = FALSE, 
...
Imp_path <- c("CD8.c02(IL7R+ Tm)", "CD8.c05(GZMK+ early Tem)", "CD8.c06(GZMK+ Tem)", 
          "CD8.c10(ZNF683+CXCR6+ Trm)", "CD8.c12(terminal Tex)")
...
column_annot <- rep(dataset$DataSets, each = 5)
Heatmap(radj, col = circlize::colorRamp2(c(-0.5, 0, 0.5), myCols), 
        rect_gp = gpar(col = "white"), 
        cluster_columns = FALSE, cluster_rows = TRUE, 
        row_names_side = "left", show_row_dend = FALSE, 
