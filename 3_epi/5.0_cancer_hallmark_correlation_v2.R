# ====================================================================
# Author: Zhou Zhe
# Function: pathway correlation of EPI
# Version: 2.0
# History: 
#   2.0: use AUCell score computed in server
#   1.0: aggregate data from different tumor
# Date: Aug 3, 2022
# ====================================================================
...
file.prefix <- "Cancer_hallmark_Cor_"
...
pathway_cor_list <- lapply(1:nrow(dataset), function(i) {
    ...
    seu_obj <- subset(seu_obj, celltype == "Malignant")
    ...
    sig_pas <- data.frame(fread(paste0("res/5_SCENIC/aucell/", tumor, "_celltypeSig_aucell.csv")),
                          row.names = 1, check.names = F)[colnames(seu_obj), ]
    sig_pas <- sig_pas[, cancer.path]
    gene.list <- c(geneIds(getGmt(paste0("res/5_SCENIC/dataset_gmt/", tumor, "_gaudeMetab.gmt"))), 
                   geneIds(getGmt(paste0("res/5_SCENIC/dataset_gmt/", tumor, "_cancerHallmarks.gmt"))))
    
    pathway_cor <- pathway_correlation(df = cbind(metab_pas, sig_pas), glist = gene.list)
    fastSave::saveRDS.pigz(pathway_cor, file = paste0("res/", dir.used, tumor, "_pathway_correlation.rds"))
    return(pathway_cor)
})
...
pathway_cor_list <- lapply(dataset$DataSets, function(tumor) {
    return(readRDS(paste0("res/", dir.used, tumor, "_pathway_correlation.rds")))
})
names(pathway_cor_list) <- dataset$DataSets
...
get_pooled_data <- function(var) {
    res <- lapply(names(pathway_cor_list), function(tumor) {
        ...
        x <- data.frame(x[setdiff(rownames(x), cancer.path), cancer.path], check.names = FALSE)
...
column_annot <- rep(dataset$DataSets, each = 7)
pdf(paste0("plot/", dir.used, file.prefix, "heatmap_by_Datasets_changed_color.pdf"), width = 9, height = 6)
...
pdf(paste0("plot/", dir.used, file.prefix, "heatmap_by_Sig_changed_color.pdf"), width = 12, height = 10)
Heatmap(radj, col = circlize::colorRamp2(c(-0.5, 0, 0.5), myCols), 
        rect_gp = gpar(col = "white"), 
        ...
        row_names_gp = tmp.text, column_names_gp = tmp.text, 
        row_title_gp = tmp.text, column_title_gp = tmp.text, 
        column_title_rot = 45, height = unit(nrow(radj)/9, "inches"), width = unit(1.5*ncol(radj)/10, "cm"), 
        heatmap_legend_param = list(title_gp = tmp.text, labels_gp = tmp.text), 
