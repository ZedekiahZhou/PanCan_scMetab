...
#   correlations calculated within each tumor
# ------------------------------------------------------------------------------------------- 

score_cor_heatmap_TF <- function(score1, # a list contains scores for cells of each sample
                              score2, 
    ...
    cor_df_comnbined <- Reduce(function(x, y) {
        x = data.frame(x, ID = rownames(x), check.names = F)
        y = data.frame(y, ID = rownames(y), check.names = F)
        res = full_join(x, y, by = "ID")
        rownames(res) = res$ID
        return(res[, setdiff(colnames(res), "ID")])
    }, x = cor_df_list)
    
    Sig_path = sort(colnames(score2[[1]]))
    column_annot <- rep(unique(sam_info$Dataset), each = length(Sig_path))
    column_split <- factor(rep(Sig_path, times = length(unique(sam_info$Dataset))), levels = colnames(score2[[1]]))
    
    if (filter_path) {
        # n_significance <- sapply(split.default(cor_df_comnbined, column_split), function(x) {
        #     rowSums((!is.na(x)) & (x != 0))
        # })
        # idx <- rowSums(n_significance >= min_datasets) > 0
        # plot_data = cor_df_comnbined[idx, ]
        
        kept_gene_list <- lapply(split.default(cor_df_comnbined, column_split), function(x) {
            g = lapply(x, function(y) {
                names(y) = rownames(x)
                y = y[!is.na(y) & abs(y) > 0.4]
                if (length(y) < 3) return(names(y)) else {
                    y = y[order(abs(y), decreasing = T)]
                    return(names(y[1:3]))
                }
            })
            # g = unique(unlist(g))
        })
        idx = unique(unlist(kept_gene_list))
        plot_data = cor_df_comnbined[idx, ]    
    } else {
    ...
    p = Heatmap(plot_data, col = circlize::colorRamp2(c(-max_cor, -max_cor/2, 0, max_cor/2, max_cor), myCols), 
                ...
                cluster_rows = F, clustering_method_rows = "ward.D2", 
